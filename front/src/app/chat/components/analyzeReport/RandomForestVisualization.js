...
function RegressorVisualization({ result, explanation }) {
  ...
  // Generate conditional styles based on residuals
  useEffect(() => {
    setTableStyles([
      {
        if: {
          ...
          data={[
            {
              ...
              mode: "markers",
              x: df_scatter.Actual,
              y: df_scatter.Predicted,
              text: df_scatter.Identifier.map(
                (id, i) => `ID: ${id}<br>Residual: ${df_scatter.Residual[i].toFixed(2)}`
              ),
          ...
          data={[
            {
              type: "scatter",
              mode: "markers",
              x: df_residual.Identifier,
              y: df_residual.Residual,
              marker: { color: "green", size: 8, opacity: 0.6 },
              hovertemplate:
                "ID: %{x}<br>Residual: %{y}<extra></extra>",
            },
            {
              type: "scatter",
              mode: "lines",
              x: [Math.min(...df_residual.Identifier), Math.max(...df_residual.Identifier)],
              y: [0, 0],
              name: "Zero Residual",
              line: { color: "red", dash: "dash" },
            },
